...
export default {
  ...
  async created() {
    if (this.isUpdate) {
      await this.loadText(this.id);
      this.payload.title = this.text.title;
      this.payload.content = this.text.content;
    }
  },
  data: () => ({
    payload: {
  ...
  methods: {
    ...mapActions("texts", ["createText", "updateText", "loadText"]),
    handleSubmit() {
      if (this.isUpdate) {
        this.updateText({ ...this.text, ...this.payload }).then(() =>
